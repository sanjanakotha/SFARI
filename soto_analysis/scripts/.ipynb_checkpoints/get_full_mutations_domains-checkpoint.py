...
# +
d_mutations = "../outputs/mutations/cds_" + variants_filename_short
folder = args.domain
...
uniprotID_ENST_mapping = pd.read_csv("../outputs/all_TFs_table_proteins.txt", sep = "\t")#10/24/24
...
for file in files:
    ...
    if (name in uniprotID_ENST_mapping_dict.keys()):
        ...
        if folder == "none":
            subprocess.call("bedtools intersect -a " + d_domains + "/" + file + " -b " + d_mutations + "/" + ENST + ".bed" + " -wb > ../outputs/mutations/domains_" + variants_filename_short + "/" + ENST + ".bed", shell = True)
        else:
            subprocess.call("bedtools intersect -a " + d_domains + "/" + file + " -b " + d_mutations + "/" + ENST + ".bed" + " -wb > ../outputs/mutations/domains_" + variants_filename_short + "/" + folder + "/" + ENST + ".bed", shell = True)
        i+=1
    else:
