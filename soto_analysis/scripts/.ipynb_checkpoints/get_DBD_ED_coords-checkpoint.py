...
with open("../outputs/all_TFs_table_proteins.txt") as f1:
    ...
    for line in f1:
        ...
        if "../outputs/mutations/cds_bed_format/" + enst not in glob.glob("../outputs/mutations/cds_bed_format/*"):
            print("no cds for " + enst)
        else:
                    ...
                    for name, domain in all_doms.items():
                        ...
                        for start_dom, end_dom in domain:
                            ...
                            for aa in range(start_dom, end_dom +1):
                                if aa <= len(dict_aa_coords):
                                    dom_pos += dict_aa_coords[aa]
                                else:
                                    print("WARNING")
                                    print(tf)
                                    print("Trying to add " + str(aa))
                                    print("but protein has length " + str(len(dict_aa_coords)))
